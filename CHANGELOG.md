...
- **Performance: deferred non-critical init**: PWA install banner and SW registration wrapped in `requestIdleCallback` (with `setTimeout` fallback) so main UI renders before secondary features load
- SW cache bumped to `pollenpal-v26.0`
...
- SW cache bumped to `pollenpal-v25.0`
...
- SW cache bumped to `pollenpal-v24.0`
